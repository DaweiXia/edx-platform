...
class Command(BaseCommand):
    ...
    def _get_users_queryset(self, initial_days):
        ...
        """
        sync_date = datetime.now().date() - timedelta(initial_days)
        self.stdout.write(u'Getting users joined on={date}'.format(date=sync_date))
        return User.objects.filter(date_joined__date=sync_date)
    ...
    def _sync_site(self, site_conf, users_queryset, users_count, contacts_batch_size):
        ...
        while offset < users_count:
            ...
            while len(users_queue) >= contacts_batch_size \
                ...
                users_queue = users_queue[contacts_batch_size:]
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=int,
            help='Number of days before today to find the day to sync',
        )
        parser.add_argument(
            '--batch-size',
            default=100,
